...
int main(void) {
  ...
  // (Prescaler + 2) -> 8% tolerance low RPM  (Prescaler + 1) -> 10% tolerance max RPM   (CPU load)
  motor.RpmFactor = ((60000000 / (escConfig()->motorPoles / 2)) / ((motorCommutationTimerHandle.Init.Prescaler + 2) / (HAL_RCC_GetSysClockFreq() * 0.000001)));
